...
public class DropTopologyTask implements Task {
  ...
  public DropTopologyTask(String topologyName, UserEntity owner) {
  ...
  public String execute() throws TaskExecuteException {
    try {
      ...
      for (Schema schema : schemas) {
        metaStore.changeSchemasToFree(metaStore.findSchema(schema.getSchemaName(),
            topology.getOwner()), topology.getId());
      }
