  ...
  public static final class Builder<T extends Stream> implements JoinStreamDeclarer<T>,
    ...
    private JoinKey[] toJoinKey(Condition condition) throws GungnirTopologyException {
      if (condition.isComplex()) {
        ...
        for (Condition c : condition.toComplex().getConditions()) {
          if (!c.isComplex()) {
        ...
        if (condition.toSimple().getField() instanceof FieldAccessor
            && condition.toSimple().getValue() instanceof FieldAccessor) {
          if (condition.getType() == ConditionType.EQ) {
            SimpleJoinKey[] joinKeys = new SimpleJoinKey[2];
            joinKeys[0] = new SimpleJoinKey((FieldAccessor) condition.toSimple().getField());
            joinKeys[1] = new SimpleJoinKey((FieldAccessor) condition.toSimple().getValue());
            return joinKeys;
          }
        }
        throw new GungnirTopologyException("Invalid join condition");
