...
public final class ArithmeticCalculator {
  ...
  public static Number compute(InternalArithNode node, GungnirTuple tuple) {
    Number left = null;
    if (node.getLeftNode().isInternal()) {
      left = compute(node.getLeftNode().toInternal(), tuple);
    } else {
      left = node.getLeftNode().toExternal().getValue(tuple);
    }

    Number right = null;
    if (node.getRightNode().isInternal()) {
      right = compute(node.getRightNode().toInternal(), tuple);
    } else {
      right = node.getRightNode().toExternal().getValue(tuple);
    }
