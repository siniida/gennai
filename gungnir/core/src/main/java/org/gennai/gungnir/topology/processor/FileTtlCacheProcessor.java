...
public class FileTtlCacheProcessor implements TtlCacheProcessor {
  ...
  private static final String DB_PATH = "ttl_cache";
  ...
  public void open(GungnirConfig config, GungnirContext context, OperatorContext operatorContext,
    ...
    try {
      ...
      sb.append(config.getString(LOCAL_DIR));
      sb.append('/');
      sb.append(DB_PATH);
      sb.append('/');
      sb.append(context.getTopologyId());
      sb.append('/');
      sb.append(operatorContext.getName() + "_" + operatorContext.getId());
      sb.append('/');
      sb.append(tupleName);
  ...
  public void put(Object key, List<Object> values) throws ProcessorException {
    ...
    try {
      table.put(key, GungnirUtils.currentTimeSec(), serializer.serialize(values));
