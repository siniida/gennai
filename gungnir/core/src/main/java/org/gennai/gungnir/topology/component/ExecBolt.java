...
public class ExecBolt extends BaseRichBolt implements GungnirComponent {
  ...
  private int id;
  private List<PartitionOperator> incomingOperators;
  private Map<String, Integer> incomingNamesIndex;
  private List<PartitionOperator> outgoingOperators;
  private Set<String> outgoingNames;
  ...
  public List<PartitionOperator> getIncomingOperators() {
  ...
  public void addIncomingOperator(PartitionOperator incomingOperator) {
    if (this.incomingOperators == null) {
      this.incomingOperators = Lists.newArrayList();
      this.incomingNamesIndex = Maps.newHashMap();
    }
    this.incomingOperators.add(incomingOperator);
    this.incomingNamesIndex.put(incomingOperator.getName(), this.incomingOperators.size() - 1);
  }
  ...
  public void addOutgoingOperators(List<PartitionOperator> outgoingOperators) {
    if (this.outgoingOperators == null) {
      ...
      this.outgoingNames = Sets.newHashSet();
    }
    for (PartitionOperator partitionOperator : outgoingOperators) {
      if (!this.outgoingNames.contains(partitionOperator.getName())) {
        this.outgoingOperators.add(partitionOperator);
        this.outgoingNames.add(partitionOperator.getName());
      }
    }
  }
  ...
  public void prepare(@SuppressWarnings("rawtypes") Map stormConf, TopologyContext context,
    ...
    for (PartitionOperator partitionOperator : incomingOperators) {
  ...
  public void execute(Tuple input) {
    ...
    PartitionOperator operator = incomingOperators.get(incomingNamesIndex.get(
