...
public class Driver implements CommandProcessor {
  ...
  private void compile(StatementEntity statement, String command)
    ...
    List<Schema> schemas =
        GungnirManager.getManager().getMetaStore().findSchemas(statement.getOwner());
    ...
    if (aliasNamesMap != null) {
      for (Map.Entry<String, String> entry : aliasNamesMap.entrySet()) {
        Schema schema = schemaRegistry.get(entry.getValue());
        if (schema != null) {
          schemaRegistry.register(entry.getKey(), schema);
        }
      }
    }
    semanticAnalyzer.setSchemaRegistry(schemaRegistry);
