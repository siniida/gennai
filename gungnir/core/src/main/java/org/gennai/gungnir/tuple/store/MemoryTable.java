...
public class MemoryTable {
    ...
    if (expireSecs > 0) {
      ...
      if (hashIndex >= 0) {
        it = delegate.subMap(i3.create(hashIndex, 0, 0), true, i3.create(hashIndex,
            GungnirUtils.currentTimeSec() - 1, -1), true).entrySet().iterator();
      } else {
        it = delegate.headMap(i3.create(hashIndex, GungnirUtils.currentTimeSec() - 1, -1),
  ...
  public int size(Object hashKey) {
    ...
    if (expireSecs > 0) {
      sz -= delegate.subMap(i3.create(hashIndex, 0, 0), true, i3.create(hashIndex,
          GungnirUtils.currentTimeSec() - 1, -1), true).size();
    }
    return sz;
  }
  ...
  private final class EntryIterator implements Iterator<Entry> {
    ...
    private EntryIterator() {
      hashIndex = -1;
      if (expireSecs > 0) {
        it = delegate.tailMap(i3.create(-1, GungnirUtils.currentTimeSec(), 0), true).entrySet()
            .iterator();
      } else {
    ...
    private EntryIterator(Object hashKey) {
      ...
      if (hashIndex != null) {
        if (expireSecs > 0) {
          it = delegate.subMap(i3.create(hashIndex, GungnirUtils.currentTimeSec(), 0), true,
  ...
  public Entry removeFirst(Object hashKey) {
    ...
    int now = GungnirUtils.currentTimeSec();
    Iterator<Map.Entry<byte[], byte[]>> it = null;
    if (hashIndex >= 0) {
      it = delegate.subMap(i3.create(hashIndex, 0, 0), true,
          i3.create(hashIndex, Integer.MAX_VALUE, -1), true).entrySet().iterator();
