...
public final class ConditionEvaluator implements Serializable {
  ...
  // TODO: No11
  private static boolean compare(ConditionType type, Object v1, Object v2) {
    switch (type) {
      case IS_NULL:
        return (v1 == null);
      case IS_NOT_NULL:
  ...
  public static boolean isKeep(Condition condition, GungnirTuple tuple) {
    if (condition.isComplex()) {
      switch (condition.getType()) {
        case AND:
          for (Condition cond : condition.toComplex().getConditions()) {
      ...
      Object v1 = condition.toSimple().getField().getValue(tuple);
      Object v2 = null;
      if (condition.toSimple().getValue() instanceof Field) {
        v2 = ((Field) condition.toSimple().getValue()).getValue(tuple);
        if (v2 == null) {
          return false;
        }
      } else {
        v2 = condition.toSimple().getValue();
      }

      return compare(condition.toSimple().getType(), v1, v2);
    }
  }
}
