...
public class TestInMemoryMetaStore {
  ...
  public void testAddTopologyToSchema() throws Exception {
    ...
    metaStore.insertSchema(schema);
    assertThat(metaStore.changeSchemasToUsed(schema, "gennai-topology"), is(true));
    assertThat(metaStore.changeSchemasToFree(schema, "gennai-topology"), is(true));

    schema.setCreateTime(new Date(System.currentTimeMillis() - 10000));
    assertThat(metaStore.changeSchemasToUsed(schema, "gennai-topology"), is(false));
    assertThat(metaStore.changeSchemasToFree(schema, "gennai-topology"), is(false));
  ...
  public void testChangeSchemaNotInMetaStore() throws Exception {
    ...
    schema.setOwner(owner);
    schema.setId("schema-id");
    metaStore.changeSchemasToUsed(schema, "gennai-topology");
  }
