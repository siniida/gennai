...
public final class Post {
    ...
    try {
      Await.ready(client.apply(request).raiseWithin(
        ...
        public void onFailure(Throwable cause) {
          LOG.error("Failed to send requests", cause);
          if (failed != null) {
      ...
      LOG.error("Send requests timed out", e);
      ...
      LOG.error("Faield to send requests", e);
  ...
  public static void main(String[] args) {
    ...
    OptionBuilder.hasArg();
    OptionBuilder.withArgName("response-time-limits");
    OptionBuilder.withDescription("Response time limits (secs)");
    ...
    int timeLimitsSecs = DEFAULT_RESPONSE_TIMEOUT_SECS;
    ...
    try {
      ...
      if (commandLine.hasOption('l')) {
        timeLimitsSecs = Integer.valueOf(commandLine.getOptionValue('l'));
      }
      ...
      if (config.getString(CLUSTER_MODE).equals(LOCAL_CLUSTER)) {
        ...
        dest = "zk!" + StringUtils.join(zkServers, ",") + "!" + STORES_NODE_PATH;
      }
      ...
      if (verbose || stats) {
        ...
        System.out.println("time limits(sec): " + timeLimitsSecs + '\n');
      }

      Post post = Post.builder().parallelism(parallelism).times(times).queueSize(queueSize)
          .timeLimitsSecs(timeLimitsSecs).verbose(verbose).stats(stats).build(dest, uri);
      post.execute();
